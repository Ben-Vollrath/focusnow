...
Deno.test("Goal is updated but not completed", async () => {
    await withAnonymousTestUser(async (client, user) => {
        ...
        assertExists(userData?.[0]);
        assertEquals(userData?.[0].id, user.id);
        assertEquals(userData?.[0].level, 1);
        assertEquals(userData?.[0].xp, 20); //10 xp from session, 10 from daily sessions challenge
    });
});
...
Deno.test("Goal is completed and grants xp", async () => {
    await withAnonymousTestUser(async (client, user) => {
        ...
        assertEquals(goal?.[0].xp_reward, 3); // 3 xp for completing the goal
        ...
        assertExists(userData?.[0]);
        assertEquals(userData?.[0].id, user.id);
        assertEquals(userData?.[0].level, 3);
        assertEquals(userData?.[0].xp, 28); //10 xp from session, 10 from daily sessions challenge, 3 from goal
    });
});
